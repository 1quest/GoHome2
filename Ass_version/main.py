...
def loopThroughRegions(data_url,
    ...
    for index, row in data_url.iterrows():
        #Base URL
        url = "https: // www.booli.se/{}/{}/?maxListPrice={}&maxLivingArea={}&minListPrice={}&minLivingArea={}&objectType=L%C3%A4genhet&page=1&upcomingSale=".format(row["Region"],
                                                                                row["RegionID"],
                                                                                maxListPrice,
                                                                                m2_max,
                                                                                minListPrice,
        ...
        for page in range(2, numberOfPages):
            ...
            ".format(row["Region"],
                ...
                newRooms=float(0.5) + float(rooms[0])object_info.set_value(index, "Number of rooms", newRooms)
                if row["Rent"].find("—") != -1:
                newRent=0
